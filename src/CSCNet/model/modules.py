...
class ConvLista_T(nn.Module):
    def __init__(self, params: ListaParams, A=None, B=None, C=None, threshold=1e-2):
        ...
        self.apply_B = nn.Conv2d(1, params.num_filters, kernel_size=params.kernel_size, stride=params.stride,
                                 bias=False)

        self.apply_C = nn.Sequential(
            torch.nn.ConvTranspose2d(params.num_filters, 1, kernel_size=params.kernel_size,
                                     stride=params.stride, bias=True),
        )

        self.apply_A.weight.data = A
        self.apply_B.weight.data = B
        self.apply_C[0].weight.data = C
